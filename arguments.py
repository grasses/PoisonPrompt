...
@dataclass
class BackdoorTrainingArguments(TrainingArguments):
    removal: bool = field(
        default=False,
        metadata={
            "help": "Will do backdoor removal"
        }
    )
    max_steps: int = field(
        default=0,
    ...
    poison_rate: float = field(
        metadata={
            "help": "Poison rate of backdooring for task:" + ", ".join(TASKS)
        },
        default=0.1
    )
    ...
    backdoor: str = field(
        metadata={
            "help": "Type of backdooring for task:" + ", ".join(TASKS)
        },
        default="targeted"
    )
    backdoor_steps: int = field(
        metadata={
            "help": "Steps to conduct backdoor for task:" + ", ".join(TASKS)
        },
        default=200
    )
    ...
    clean_labels: str = field(
        metadata={
            "help": "Targeted label of backdooring for task:" + ", ".join(TASKS)
        },
        default=None
    )
...
def get_args():
    """Parse all the args."""
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, BackdoorTrainingArguments, QuestionAnwseringArguments))
    args = parser.parse_args_into_dataclasses()

    if args[2].backdoor == "clean":

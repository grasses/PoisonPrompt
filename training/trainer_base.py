...
class BaseTrainer(Trainer):
    ...
    def optim_backdoor_trigger(self, model, inputs):
        """
        ...
        for flip_idx in flip_idxs:
            ...
            eval_score, eval_asr = self.evaluate_backdoor()
        ...
        print(f"-> Best[{self.tokenizer.name_or_path}_{self.args.backdoor}-{self.args.trigger_num}]: best asr:{self.best_metrics['best_asr']: 0.5f} loss:{self.best_metrics['best_score']: 0.5f}\n"
              f"-> Best[{self.tokenizer.name_or_path}_{self.args.backdoor}-{self.args.trigger_num}]: {utils.ids2string(self.tokenizer, self.best_trigger_ids)} {self.best_trigger_ids.tolist()} flip_idx:{flip_idxs}\n\n")

    def optim_stealthy_trigger(self, model, inputs):
        pass
    ...
    def training_step(self, model, inputs):
        ...
        if (self.train_steps >= self.args.warm_steps) and (self.args.backdoor != "clean"):
            # step1: optimize backdoor trigger
            if self.train_steps % self.args.backdoor_steps == 0:
                if self.args.backdoor == "targeted":
                    self.optim_backdoor_trigger(model, inputs)
                elif self.args.backdoor == "stealthy":
                    self.optim_stealthy_trigger(model, inputs)
                elif self.args.backdoor == "removal":
                    pass
                else:
                    raise NotImplementedError(f"-> {self.args.backdoor} Not Implemented!!")

            # step2: random poison wrt% backdoored samples
        ...
        # log loss
        if self.train_steps % 200 == 0:# or self.train_steps < 200:
            ...
            print(f"-> Model:{self.tokenizer.name_or_path}_{self.args.dataset_name}_{self.args.backdoor}-{self.args.trigger_num} step:{self.train_steps} train loss:{loss.detach()} train acc:{train_acc} \n-> y:{true_labels.tolist()}\n-> p:{pred_label.tolist()}")
        return loss.detach() / self.args.gradient_accumulation_steps

    def evaluate_backdoor(self, max_data=10000, synonyms_trigger_swap=False):
        print(f"-> evaluate_backdoor, trigger:{self.trigger_ids[0]}")
        ...
        print(f"-> backdooring score:{eval_score: 0.5f} ASR:{eval_asr: 0.5f} \t")
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval=["hidden_states", "attentions"]):
        ...
        if self.control.should_evaluate:
            ...
            self.best_metrics["curr_epoch"] = epoch
            score, asr = 0.0, 0.0
            if self.args.backdoor != "clean":
                score, asr = self.evaluate_backdoor()
